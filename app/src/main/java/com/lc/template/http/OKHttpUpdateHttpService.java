...
public class OKHttpUpdateHttpService implements IUpdateHttpService {
    ...
    public void asyncGet(@NonNull String url, @NonNull Map<String, Object> params, final @NonNull Callback callBack) {
        ...
        OkHttpUtils.get().url(url).params(transform(params)).build().execute(new StringCallback() {
            @Override
...
//        这里默认post的是Form格式，使用json格式的请修改 post -> postString
        ...
        if (mIsPostJson) {
            requestCall = OkHttpUtils.postString().url(url).content(GsonUtils.toJson(params)).mediaType(MediaType.parse("application/json; charset=utf-8")).build();
        } else {
            requestCall = OkHttpUtils.post().url(url).params(transform(params)).build();
        }
